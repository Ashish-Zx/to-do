...
function Main() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const { data } = await client.get(`/lists/getTasks/${userId}`);
    ...
    const handleAddTask = async (e) => {
        ...
        try {
            const { data } = await client.post('/lists/addTask', { userId, title: newTask.title, description: newTask.description });
            const l = data?.list;
            if (l) {
                setTasks([...tasks, { id: l._id, title: l.title, description: l.description || '', completed: l.status === 'completed' }]);
    ...
    const handleEditTask = async (e) => {
        e.preventDefault();
        try {
            await client.put(`/lists/updateTask/${currentTask.id}`, { title: currentTask.title, description: currentTask.description, status: currentTask.completed ? 'completed' : 'active' });
    ...
    const handleDeleteTask = async (id) => {
        ...
        try {
            await client.delete(`/lists/deleteTask/${id}`, { data: { userId } });
    ...
    const displayName = localStorage.getItem('userName') || localStorage.getItem('userEmail') || 'User';
    return (
        <>
            <AuthNavbar userName={displayName} />
